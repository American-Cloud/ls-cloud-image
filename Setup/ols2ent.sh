...
CONF_URL='https://raw.githubusercontent.com/American-Cloud/ls-cloud-image/master/Setup/conf/ols2ent'
...
lsws_conf_file() {
    ...
    wget -O $LS_DIR/conf/httpd_config.xml https://raw.githubusercontent.com/American-Cloud/ls-cloud-image/master/Setup/conf/ols2ent/httpd_config.xml
    check_return
    if [[ $pkg_tool == "apt" ]] ; then
...
write_apache_conf() {
    ...
    if [[ ! -f $STORE_DIR/httpd.conf ]] ; then
        wget -q -O $STORE_DIR/httpd.conf https://raw.githubusercontent.com/American-Cloud/ls-cloud-image/master/Setup/conf/ols2ent/httpd.conf
        check_return
    fi
    ...
    wget -q -O $STORE_DIR/conf/vhosts/${domains[i]}/${domains[i]}.conf https://raw.githubusercontent.com/American-Cloud/ls-cloud-image/master/Setup/conf/ols2ent/example.conf
    check_return
